...
#
#
# def equalize(img): #pgmagick perform histogram equalization to the image.
...
batch_size_val = 50  # how many to process together
epochs_val = 10
imageDimensions = (32, 32, 3)  # Updated for color images
testRatio = 0.2  # if 1000 images split will be 200 for testing
validationRatio = 0.2  # if 1000 images, 20% of remaining 800 will be for validation
###################################################
...
for x in range(0, len(myList)):
    ...
    for y in myPicList:
        curImg = cv2.imread(path + "/" + str(count) + "/" + y)
        curImg = cv2.resize(curImg, (imageDimensions[0], imageDimensions[1]))  # Resize images to desired dimensions
        images.append(curImg)
        classNo.append(count)
    print(count, end=" ")
    count += 1
...
############################### TO CHECK IF NUMBER OF IMAGES MATCHES THE NUMBER OF LABELS FOR EACH DATA SET
print("Data Shapes")
print("Train", end="")
print(X_train.shape, y_train.shape)
print("Validation", end="")
